...
class softActorCriticContinuousPER(nn.Module):
    def __init__(self, stateDim,
        ...
        self.expReplay = PrioritizedExperienceReplay(bufferSize, stateDim, actionDim, bufferAlpha, bufferBeta, bufferDeltaBeta)
    ...
    def updateCritic(self, s, a, r, d, s_, w):
        ...
        Returns:
            torch.Tensor: TD Errors for each transition in the minibatch.
        -------------------------------------
        v_ = min_i Q_i(s_, a_) - α * log(π(a_ | s_))
        Target Q-value: q_ = r + γ * v_ * (1 - d) (where γ is the discount factor)
        TD error: tde = |q - q_|
        Loss function: L = WMSE(TDError, w) Where WMSE(e, w) is Weighted MSE; e is the error and w is its weight.
        '''
    ...
    def learn(self, minibatchSize, numMiniBatch):
        '''
        Performs a learning step by updating the actor, critic, alpha, and target networks based on prioritized experience replay (PER) samples minibatch transitions from the experience replay buffer, considering their importance weights.

        Args:
            minibatchSize (int): Size of each minibatch sampled from the experience buffer.
