...
agent = deepQLearningSingle(stateDim, actionDim, hiddenDim=32, numHiddenLayers=2, lr=1e-3)
...
for ii in range(numEpisodes):
    ...
    while not d:
        ...
        print("----------[ Playing Phase: Episode: {:04}/{:04}, Step: {:04}, Reward: {:04.3f}, Best Reward: {:04.3f} ]----------".format(ii+1, numEpisodes, jj, rewardHist[ii], bestScore), end='\r', flush=True)
    if rewardHist[ii].sum(-1) >= bestScore:
        agent.save('best')
        bestScore = rewardHist[ii].sum(-1)
    print("----------[ Playing Phase: Episode: {:04}/{:04}, Step: {:04}, Reward: {:04.3f}, Best Reward: {:04.3f} ]----------".format(ii+1, numEpisodes, jj, rewardHist[ii], bestScore))
    agent.save('last')
env.close()
