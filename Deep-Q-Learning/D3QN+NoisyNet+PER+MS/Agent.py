...
class deepQLearningDoubleDuellingNoisyPERMultistep(nn.Module):
    """
    ...
    def __init__(self, stateDim,
        ...
        super(deepQLearningDoubleDuellingNoisyPERMultistep, self).__init__()
        ...
        self.expReplay = MultiStepPrioritizedExperienceReplay(bufferSize, stateDim, 1, numSteps, bufferAlpha, bufferBeta, bufferDeltaBeta)
        
    def updateCritic(self, s, a, r, d, s_, w, n):
        """
        ...
        r_ = (r * self.gamma ** torch.arange(self.numSteps, device = r.device)).sum(-1, keepdim=True)
        targetQ = (r_ + self.gamma**n * self.critic_(s_)[torch.arange(len(a_)), a_.squeeze()].unsqueeze(-1) * (1 - d)).squeeze()
    ...
    def learn(self, minibatchSize, numMiniBatch):
        ...
        """
        state, action, reward, done, state_, weight, nSteps = self.expReplay.sample(minibatchSize, numMiniBatch)
        for jj in range(numMiniBatch):
            ...
            n = nSteps[jj].to(device)
            
            tde = self.updateCritic(s, a, r, d, s_, w, n)
