...
class MultiStepPrioritizedExperienceReplay:
    def __init__(self, bufferSize, stateDim, actionDim, numSteps, alpha = 1.0, beta = 1.0, deltaBeta = 1e-3):
        """
        ...
        self.reward = torch.zeros((bufferSize, numSteps), dtype=torch.float32)
        ...
        self.nSteps = torch.zeros((bufferSize, 1), dtype=torch.float32)
    ...
    def store(self, state, action, reward, done, state_):
        ...
        self.done[ii] = done
        self.reward[ii] = 0
        self.reward[ii, 0] = reward
        self.state_[ii] = torch.from_numpy(state_)
        for jj in range(1, min(self.bufferHead + 1, self.numSteps)):
            self.reward[ii - jj, jj] = 0 if self.done[ii - jj] == 1 else reward
            if done: self.done[ii - jj] = done
            self.state_[ii - jj] = torch.from_numpy(state_)
            self.nSteps[ii - jj] += 1
    ...
    def sample(self, minibatchSize, numMinibatch):
        ...
        return (state.reshape(numMinibatch, minibatchSize, *state.shape[1:]),
                ...
                done.reshape(numMinibatch, minibatchSize, *done.shape[1:]),
                state_.reshape(numMinibatch, minibatchSize, *state_.shape[1:]),
                isw.reshape(numMinibatch, minibatchSize, *isw.shape[1:]),
                nSteps.reshape(numMinibatch, minibatchSize, *nSteps.shape[1:]))
