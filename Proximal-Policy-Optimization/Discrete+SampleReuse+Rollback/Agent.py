...
"""
Proximal Policy Optimization with Rollback Surrogate (PPO-Clip) with Generalized Advantage Estimation (GAE) and Sample Reuse for environments with discrete action-space.
...
"Truly Proximal Policy Optimization" (https://arxiv.org/abs/1903.07940) by Yuhui Wang et al.
...
"""
class ppoRollbackDiscreteSampleReuse(nn.Module):
    def __init__(self, stateDim = 96,
                 ...
                 valueLr=2.5e-4,
                 policyLr=2.5e-4):
        super(ppoRollbackDiscreteSampleReuse, self).__init__()
    ...
    def updatePolicy(self, s, a, p, p_, adv):
        ...
        r_ = (p_ - p).exp()
        L = -torch.where((r - r_).abs() <= self.eps, r * adv, -self.alpha * r * adv).mean()
        L.backward()
