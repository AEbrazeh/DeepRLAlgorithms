...
class ppoRollbackDiscrete(nn.Module):
    """
    Proximal Policy Optimization with Rollback Surrogate (PPO-Rollback) with Generalized Advantage Estimation (GAE) for environments with discrete action-space.
    Based on "Proximal Policy Optimization Algorithms" (https://arxiv.org/abs/1707.06347) by John Schulman et al., "Truly Proximal Policy Optimization" (https://arxiv.org/abs/1903.07940) by Yuhui Wang et al. and "High-Dimensional Continuous Control Using Generalized Advantage Estimation" (https://arxiv.org/abs/1506.02438) by John Schulman et al.
    ...
    Args:
        ...
        alpha (float, optional): Rollback parameter. Defaults to 0.01.
        eps (float, optional): Rollback threshold parameter for PPO. Defaults to 0.2.
    ...
    def __init__(self, stateDim,
                 ...
                 valueLr=2.5e-4,
                 policyLr=2.5e-4):
        super(ppoRollbackDiscrete, self).__init__()
    ...
    def updatePolicy(self, s, a, p, adv):
        """
        Updates the policy network by computing the PPO-Rollback loss and performing gradient descent.
        ...
        L = -torch.where((r - 1).abs() <= self.eps, r * adv, -self.alpha * r * adv).mean()
        L.backward()
